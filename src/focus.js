...
canvas.addEventListener('mousemove', async (e) => {
    ...
    if (x >= 0 && x < GRID_SIZE && y >= 0 && y < GRID_SIZE) {
        try {
            ...
            const pixel = await getPixelDetails(contract, x, y);
            const isPixelSet = pixel.topLocker !== '0x0000000000000000000000000000000000000000' && pixel.highestAmountLocked !== '0';
            document.getElementById('topLockerValue').innerText = isPixelSet ? pixel.topLocker : 'N/A';
            document.getElementById('highestAmountLockedValue').innerText = isPixelSet ? (Number(pixel.highestAmountLocked) / 10**18).toString() : 'N/A';
        } catch (error) {
